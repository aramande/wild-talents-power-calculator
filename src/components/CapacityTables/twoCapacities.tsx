...
const TwoCapacities: React.FC<TwoCapacitiesProps> = ({ reduced, boosters, firstCapacity, secondCapacity, offCapacity }) => {
  const header = [];
  const rows = [];
  header.push(<tr key={'headerRow'} className='border-bottom border-color--primary border-bottom--thick'><td>{firstCapacity.getType()}</td><td>{secondCapacity.getType()}</td>{offCapacity.isMaxed() && <td>{offCapacity.getType()}</td>}</tr>);
  for (let rowIndex = 0; rowIndex <= boosters; rowIndex++) {
    rows.push(<tr  className='border-bottom border-color--primary border-right border-left' key={'row'+rowIndex}>
      <td>{firstCapacity.getValue() * Math.pow(10, rowIndex) * Math.pow(10, -reduced)} {firstCapacity.getMeasure()}</td>
