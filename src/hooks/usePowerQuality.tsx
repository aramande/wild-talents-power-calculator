...
import { IPowerModifier, IPowerQuality, TCapacity, TType } from '../interfaces/power.interface';
...
type SetPowerType = Action<typeof PowerQualityActionKind.SET_POWER_TYPE, TType>;
function reduceSetPowerTypeAction(state: IPowerQuality, action: SetPowerType): IPowerQuality {
  if (action.payload === undefined) return state;
...
function powerQualityReducer(state: IPowerQuality, action: PowerQualityActions): IPowerQuality {
  switch (action.type) {
    ...
    case PowerQualityActionKind.SET_POWER_TYPE:
      return reduceSetPowerTypeAction(state, action);
